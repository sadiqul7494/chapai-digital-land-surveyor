...
module.exports = async (req, res) => {
  ...
  try {
    // ---- Single booking: GET/PATCH/DELETE by ?id= ----
    if (id) {
      if (req.method === 'GET') {
        const rows = await sql`SELECT * FROM bookings WHERE id = ${id}`;
        if (!rows[0]) return res.status(404).json({ error: 'বুকিং পাওয়া যায়নি।' });
        const files = await sql`SELECT name, mime_type, size_bytes, data_url FROM booking_files WHERE booking_id = ${id}`;
        return res.status(200).json({ ...rows[0], files });
      }
      if (req.method === 'PATCH') {
    ...
    // ---- Collection: GET (list/filter) / POST (create) ----
    if (req.method === 'GET') {
      let rows;
      if (date) {
    ...
    if (req.method === 'POST') {
      ...
      const rows = await sql`
        ...
        VALUES
          (${newId}, ${b.name}, ${b.phone}, ${b.upazila || null}, ${b.size || null}, ${b.service || null},
      ...
      if (Array.isArray(b.attachments) && b.attachments.length) {
        for (const f of b.attachments) {
          await sql`
            ...
            VALUES (${newId}, ${f.name}, ${f.type || null}, ${Math.round(f.size || 0)}, ${f.dataUrl})`;
        }
      }

      if (b.promo) {
        await sql`UPDATE promo_codes SET used = TRUE, used_by = ${newId} WHERE code = ${b.promo}`;
      }
